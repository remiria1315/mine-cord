...
const DISCORD_WEBHOOK_URL = 'webhookのurl！';
const DISCORD_CHANNEL_ID = '連携するチャンネルのid！';
const DISCORD_BOT_TOKEN = 'botのtoken！';
const ADMIN_ROLES = ['コマンドを許可するロールのid！'];
...
async function generateIconFromSkin(imageBuffer, outputPath) {
  ...
  ctx.drawImage(skin, 8 * factor, 8 * factor, 8 * factor, 8 * factor, 1 * scale, 1 * scale, 8 * scale, 8 * scale);
  ctx.drawImage(skin, 40 * factor, 8 * factor, 8 * factor, 8 * factor, 0.5 * scale, 0.5 * scale, 9 * scale, 9 * scale);
...
(async () => {
  const mcbeClient = new Server({ port: MCBE_PORT, debug: true, commandVersion: "1.19.70" });
    ...
    for (let raw of rawtext) {
      ...
      switch (raw.keys[0]) {
        ...
        case "selector": {
          ...
          if (res.statusCode == 0) text += res.victim.join(", ");
          break;
        }
        case "score": {
          ...
          if (res.statusCode == 0) text += value;
          break;
        }
      }
    }
  ...
  mcbeClient.on(ServerEvent.PlayerMessage, async event => {
    ...
    if (type == "tell") type = rawutil.isRawText(message) ? "tellraw" : "tell";
    switch (type) {
      case "me":
        sendDiscordMessage(username, "* " + message);
        console.log("* " + message);
  ...
  discordBot.on('messageCreate', async message => {
    ...
    if (message.content.startsWith("/")) {
      ...
      let i = 0;
      let rep = await message.reply(
        ADMIN_ROLES.some(roleId => message.member.roles.cache.has(roleId))
          ? "実行中..."
          : "権限が足りません"
      );
      if (!ADMIN_ROLES.some(roleId => message.member.roles.cache.has(roleId))) return;
      ...
      try {
        ...
        embed = new EmbedBuilder()
          .setTitle(`結果:`)
          .setColor(statusCode == 0 ? "Green" : "Red")
          .setDescription(`\`\`\`json\n${res}\n\`\`\``)
          .setFooter({ text: "by " + name, iconURL: message.member.displayAvatarURL() });
      } catch (error) {
        embed = new EmbedBuilder()
          .setTitle(`結果:`)
          .setColor("#000000")
